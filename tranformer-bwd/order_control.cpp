...
extern float grad_cores_attnq[num_cores * WD];
extern float grad_cores_attnk[num_cores * WD];
extern float grad_cores_attnv[num_cores * WD];
extern float grad_cores_attnfc[num_cores * WD];
#endif
...
void order_control_tt_grad(
    ...
    */
    ...
    //gradient_6
    contract(tt_cores, tt_cores, buffer_left[1], WD * 3, WD * 4, 0, tt_ranks[3], tt_shapes[3], tt_shapes[4], tt_ranks[5], tt_ranks[4]); //G4G5 10,8,8,10
    ...
    //gradient_4
    contract(tt_cores, tt_cores, buffer_left[1], WD * 4, WD * 5, 0, tt_ranks[4], tt_shapes[4], tt_shapes[5], tt_ranks[6], tt_ranks[5]); //G5G6 10,8,8,1
    ...
    //gradient_3
    contract(tt_cores, tt_cores, buffer_left[0], 0, WD * 1, 0, tt_ranks[0], tt_shapes[0], tt_shapes[1], tt_ranks[2], tt_ranks[1]); //G1G2 1,16,16,10
    ...
    //gradient_1
    contract(tt_cores, tt_cores, buffer_left[0], WD * 1, WD * 2, 0, tt_ranks[1], tt_shapes[1], tt_shapes[2], tt_ranks[3], tt_ranks[2]); //G2G3
    ...
    //gradient_2
    contract_last_left(tt_cores, buffer_right[0], buffer_left[0], 0, 0, 0, tt_ranks[1], tt_shapes[1], tt_shapes[2], tt_ranks[2], tt_shapes[0]);//(G4G5G6dy)XG1 10,8,8,10,12
...
#pragma HLS interface mode=bram port=layernorm_b
    ...
    contract(tt_cores_attnq, tt_cores_attnq, buffer_q[0], 0, WD * 1, 0, 1, attn_shape[0], attn_shape[1], tt_ranks[2], tt_ranks[1]); //G1G2, [1,12,8,10]
    contract(buffer_q[0], tt_cores_attnq, buffer_q[1], 0, WD * 2, 0, attn_shape[0], attn_shape[1], attn_shape[2], tt_ranks[3], tt_ranks[2]); //G1G2G3, [12,8,8,10]
    contract_middle(input, buffer_q[1], buffer_q[0], 0, 0, 0, seq_len, tt_ranks[3], attn_shape[0] * attn_shape[1] * attn_shape[2]);//G1G2G3X
    contract(tt_cores_attnq, tt_cores_attnq, buffer[0], WD * 4, WD * 5, 0, tt_ranks[4], attn_shape[4], attn_shape[5], tt_ranks[6], tt_ranks[5]); //G5G6, [10,8,8,1]
    contract(tt_cores_attnq, buffer[0], buffer_q[1], WD * 3, 0, 0, tt_ranks[3], attn_shape[3], attn_shape[4], attn_shape[5], tt_ranks[4]); //G4G5G6, [10,12,8,8]
    contract_last(buffer_q[0], buffer_q[1], input_q, 0, 0, 0, seq_len, attn_shape[3], attn_shape[4], attn_shape[5], tt_ranks[3]); //G1G2G3XG4G5G6 [32,12,8,8,10]
Loop_Bq_I:
...
#pragma HLS loop_tripcount max=seq_len
    ...
    contract(tt_cores_attnk, tt_cores_attnk, buffer_k[0], 0, WD * 1, 0, 1, attn_shape[0], attn_shape[1], tt_ranks[2], tt_ranks[1]); //G1G2, [1,8,8,10,10]
    contract(buffer_k[0], tt_cores_attnk, buffer_k[1], 0, WD * 2, 0, attn_shape[0], attn_shape[1], attn_shape[2], tt_ranks[3], tt_ranks[2]); //G1G2G3, [8,8,12,10,10]
    contract_middle(input, buffer_k[1], buffer_k[0], 0, 0, 0, seq_len, tt_ranks[3], attn_shape[0] * attn_shape[1] * attn_shape[2]);//G1G2G3X
    contract(tt_cores_attnk, tt_cores_attnk, buffer[0], WD * 4, WD * 5, 0, tt_ranks[4], attn_shape[4], attn_shape[5], tt_ranks[6], tt_ranks[5]); //G5G6, [10,8,12,1,10]
    contract(tt_cores_attnk, buffer[0], buffer_k[1], WD * 3, 0, 0, tt_ranks[3], attn_shape[3], attn_shape[4], attn_shape[5], tt_ranks[4]); //G4G5G6, [10,8,8,12,10]
...
#pragma HLS loop_tripcount max=seq_len
    ...
    contract(tt_cores_attnv, tt_cores_attnv, buffer_v[0], 0, WD * 1, 0, 1, attn_shape[0], attn_shape[1], tt_ranks[2], tt_ranks[1]); //G1G2
    contract(buffer_v[0], tt_cores_attnv, buffer_v[1], 0, WD * 2, 0, attn_shape[0], attn_shape[1], attn_shape[2], tt_ranks[3], tt_ranks[2]); //G1G2G3
    contract_middle(input, buffer_v[1], buffer_v[0], 0, 0, 0, seq_len, tt_ranks[3], attn_shape[0] * attn_shape[1] * attn_shape[2]);//G1G2G3X
    contract(tt_cores_attnv, tt_cores_attnv, buffer[0], WD * 4, WD * 5, 0, tt_ranks[4], attn_shape[4], attn_shape[5], tt_ranks[6], tt_ranks[5]); //G5G6
    contract(tt_cores_attnv, buffer[0], buffer_v[1], WD * 3, 0, 0, tt_ranks[3], attn_shape[3], attn_shape[4], attn_shape[5], tt_ranks[4]); //G4G5G6
...
#pragma HLS loop_tripcount max=seq_len
    ...
    contract(tt_cores_attnfc, tt_cores_attnfc, buffer_o[0], 0, WD * 1, 0, 1, attn_shape[0], attn_shape[1], tt_ranks[2], tt_ranks[1]); //G1G2
    contract(buffer_o[0], tt_cores_attnfc, buffer[0], 0, WD * 2, 0, attn_shape[0], attn_shape[1], attn_shape[2], tt_ranks[3], tt_ranks[2]); //G1G2G3
    ...
    contract(tt_cores_attnfc, tt_cores_attnfc, buffer[1], WD * 4, WD * 5, 0, tt_ranks[4], attn_shape[4], attn_shape[5], tt_ranks[6], tt_ranks[5]); //G5G6
    contract(tt_cores_attnfc, buffer[1], buffer_o[1], WD * 3, 0, 0, tt_ranks[3], attn_shape[3], attn_shape[4], attn_shape[5], tt_ranks[4]); //G4G5G6
