...
void order_control_ff1(
    ...
    contract_last_left(buffer_w1[1], input_ff1, buffer_w1[0], 0, 0, 0, tt_ranks[4], pff_shape_0[0], pff_shape_0[1], pff_shape_0[2], seq_len);//(G4G5G6dy)X 10,12,8,8
...
void order_control_ff2(
    ...
    contract_last_left(buffer_w4[1], input_ff2, buffer_w4[0], 0, 0, 0, tt_ranks[4], pff_shape_1[0], pff_shape_1[1], pff_shape_1[2], seq_len);//(G4G5G6dy)X 10,16,16,12
...
void order_control_tt_grad(
    ...
    */
    ...
    //gradient_6
    contract(tt_cores, tt_cores, buffer_left[1], WD_ATTN * 3, WD_ATTN * 4, 0, tt_ranks[3], tt_shapes[3], tt_shapes[4], tt_ranks[5], tt_ranks[4]); //G4G5 10,8,8,10
    contract_middle_left(buffer_left[1], buffer_right[1], grad_cores, 0, 0, WD_ATTN * 5, tt_ranks[3], tt_shapes[5], tt_ranks[5] * tt_shapes[3] * tt_shapes[4]);//(G1G2G3X)dyG4G5=gradient_6
    //gradient_4
    contract(tt_cores, tt_cores, buffer_left[1], WD_ATTN * 4, WD_ATTN * 5, 0, tt_ranks[4], tt_shapes[4], tt_shapes[5], tt_ranks[6], tt_ranks[5]); //G5G6 10,8,8,1
    contract_right(buffer_right[1], buffer_left[1], grad_cores, 0, 0, WD_ATTN * 3, tt_ranks[3], tt_shapes[3], tt_ranks[4], tt_ranks[6], tt_shapes[4] * tt_shapes[5]);//(G1G2G3X)dyG5G6=gradient_4
    //gradient_5
    clear(buffer_left[1], d_model * 10);
    contract_left(tt_cores, buffer_right[1], buffer_left[1], WD_ATTN * 3, 0, 0, tt_ranks[3], 1, tt_shapes[4], tt_shapes[5], tt_ranks[4] * tt_shapes[3]); //(G1G2G3X)dyG4 10,8,12
    contract_right(buffer_left[1], tt_cores, grad_cores, 0, WD_ATTN * 5, WD_ATTN * 4, tt_ranks[4], tt_shapes[4], tt_ranks[5], tt_ranks[6], tt_shapes[5]);//(G1G2G3X)dyG4G6=gradient_5

    //gradient_3
    contract(tt_cores, tt_cores, buffer_left[0], 0, WD_ATTN * 1, 0, tt_ranks[0], tt_shapes[0], tt_shapes[1], tt_ranks[2], tt_ranks[1]); //G1G2 1,16,16,10
    contract_left(buffer_left[0], buffer_right[0], grad_cores, 0, 0, WD_ATTN * 2, tt_ranks[0], tt_ranks[2], tt_shapes[2], tt_ranks[3], tt_shapes[0] * tt_shapes[1]);//(G4G5G6dy)XG1G2=gradient_3
    //gradient_1
    contract(tt_cores, tt_cores, buffer_left[0], WD_ATTN * 1, WD_ATTN * 2, 0, tt_ranks[1], tt_shapes[1], tt_shapes[2], tt_ranks[3], tt_ranks[2]); //G2G3
    ...
    contract_right(buffer_left[0], tt_cores, grad_cores, 0, WD_ATTN * 2, WD_ATTN * 1, tt_ranks[1], tt_shapes[1], tt_ranks[0], tt_ranks[3], tt_ranks[2] * tt_shapes[2]);//(G4G5G6dy)XG1G3=gradient_2
}
...
#pragma HLS loop_tripcount max=seq_len
        input_ff2[i] = 0.5 * output_1[i] * (1.0 + tanhf(sqrtf(2.0 / M_PI) * (output_1[i] + 0.044715 * powf(output_1[i], 3))));
    }
...
#pragma HLS loop_tripcount max=seq_len
        Loop_B2_J :
        for (int j = 0; j < d_model * Batchsize; j++) {
            output_2[i * d_model + j] += bias_ff2[j] + input_ff1[i * d_model + j];
        }
    }
...
Loop_GRAD_ATTN:
    for (int i = 0; i < seq_len * d_model; i++) {
            grad_output[i] += grad_output2[i];
    }

    /* Train FC Layers */
    order_control_ff1(tt_cores_ff1, tt_ranks, pff_shape_0, grad_cores_ff1, buffer_w1, buffer_w2); //[0]G1G2G3X,[1]G1G2G3;[0]G5G6,[1]G4G5G6;
    order_control_ff2(tt_cores_ff2, tt_ranks, pff_shape_1, grad_cores_ff2, buffer_w4, buffer_w3); //[0]G1G2G3X,[1]G1G2G3;[0]xxx,G4G5G6;
