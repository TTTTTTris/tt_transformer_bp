...
Loop_I:
    for (int i = 0; i < I; i++) {
#pragma HLS loop_tripcount min=10 max=12
        Loop_J:
        for (int j = 0; j < J; j++) {
...
#pragma HLS loop_tripcount min=1 max=12
                    ...
                    for (int r = 0; r < R; r++) {
#pragma HLS loop_flatten off
#pragma HLS loop_tripcount min=10 max=64
...
#pragma HLS loop_flatten off
#pragma HLS pipeline off
#pragma HLS loop_tripcount max=d_hidden
...
#pragma HLS loop_tripcount max=10
            ...
            for (int m = 0; m < M; m++) {
#pragma HLS loop_flatten off
#pragma HLS loop_tripcount max=d_model
                //float res_l = tensor_l[i * M + m + l_offset]; //IM=IJKL
